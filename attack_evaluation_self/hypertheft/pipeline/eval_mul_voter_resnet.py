...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate a ResNet HyperTheft checkpoint with mul-voter."
    )
    parser.add_argument("--run_dir", required=True, type=str)
    parser.add_argument("--ckpt", required=True, type=str)
    parser.add_argument("--task_name", default="task28_8vs9", type=str)
...
def load_heldout_task(cfg, mode, task_name):
    ...
    if len(tasks) != 1:
        raise RuntimeError(
            f"Expected exactly one heldout task named {task_name}, found {len(tasks)}"
        )
    return tasks[0]
...
def build_codes(cfg, hypertheft, task, device):
    codes_list = []
    for seed_id, (inter_out, _, _) in enumerate(task["eval_seed_loader"]):
        if seed_id >= cfg.n_seed:
            break
        inter_out = inter_out[: cfg.batch_size].to(device, non_blocking=True)
        codes_list.append(hypertheft.mixer(inter_out))
    if len(codes_list) != cfg.n_seed:
        raise RuntimeError(
            f"Only collected {len(codes_list)} seed batches, expected {cfg.n_seed}"
        )
    return codes_list
...
def evaluate_votes(cfg, hypertheft, task, codes_list, num_vote_list, ckpt_name, epoch):
    ...
    for num_vote in num_vote_list:
        params_list = [
            hypertheft.generator(torch.cat(codes_list, -1)) for _ in range(num_vote)
        ]
        ...
        for _, images, labels in task["eval_loader"]:
            ...
            counter = torch.zeros(
                cfg.batch_size, labels.size(0), cfg.n_class, device=device
            )
            for params in params_list:
                ...
                for layers in zip(*params):
                    ...
                    counter[
                        batch_id,
                        torch.arange(pred.size(0), device=device),
                        pred,
                    ] += 1
                    batch_id += 1
            voted_pred = counter.argmax(-1)
            voted_correct = voted_pred.eq(labels.repeat(cfg.batch_size, 1))
            threshold = math.floor(num_vote * 0.75)
...
def main():
    ...
    if hasattr(models, "infer_ngen"):
        ...
        if int(cfg.ngen) != expected_ngen:
            raise RuntimeError(
                f"Run config ngen={cfg.ngen} does not match models.{cfg.target_model} "
                f"expected ngen={expected_ngen}"
            )
