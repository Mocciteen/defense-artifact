...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train HyperTheft on ResNet CIFAR binary-task traces."
    )
...
def train_one_epoch(args, hypertheft, tasks, optimizers):
    ...
    for task in tasks:
        ...
        for traces, images, labels in task["train_loader"]:
            del traces
...
def evaluate(args, hypertheft, tasks, num_voters):
    ...
    for task in tasks:
        ...
        for voter_count in num_voters:
            params_list = [
                generator(torch.cat(codes_list, -1)) for _ in range(voter_count)
            ]
