...
std::optional<Tensor> MaybeConvertInputTensorWithDither(const DLTensor* src, const Device& dev,
                                                        Allocator* alloc,
                                                        ffi::String scope = "global") {
  if (!inputzerodither::ShouldApply(src)) {
    return std::nullopt;
  }

  ffi::Shape shape(src->shape, src->shape + src->ndim);
  if (dev.device_type == kDLCPU) {
    Tensor dst = alloc->Empty(shape, src->dtype, dev, scope);
    if (inputzerodither::TryCopyFromTensor(src, const_cast<DLTensor*>(dst.operator->()))) {
      return dst;
    }
    return std::nullopt;
  }

  Tensor staging = Tensor::Empty(shape, src->dtype, DLDevice{kDLCPU, 0});
  if (!inputzerodither::TryCopyFromTensor(src, const_cast<DLTensor*>(staging.operator->()))) {
    return std::nullopt;
  }

  Tensor dst = alloc->Empty(shape, src->dtype, dev, scope);
  dst.CopyFrom(staging);
  return dst;
}

Tensor ConvertTensorToDevice(Tensor src, const DLDevice& dev, Allocator* alloc,
                             ffi::String scope = "global") {
  if (auto opt = MaybeConvertInputTensorWithDither(src.operator->(), dev, alloc, scope)) {
    return opt.value();
  }
...
ffi::Any ConvertArgToDevice(ffi::AnyView input, Device dev, Allocator* alloc) {
  ...
  if (auto opt_obj = input.as<ObjectRef>()) {
    ...
    if (auto opt = MaybeConvertInputTensorWithDither(tensor, dev, alloc)) {
      ret = opt.value();
    } else {
