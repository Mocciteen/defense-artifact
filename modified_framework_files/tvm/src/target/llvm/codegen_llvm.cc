...
enum class ReluLow12PatchKind {
  kNone,
  kRelu,
  kRelu6,
};

bool IsConstScalarFloatLike(const PrimExpr& expr, double value) {
  if (const auto* float_imm = expr.as<FloatImmNode>()) {
    return float_imm->value == value;
  }
  if (const auto* int_imm = expr.as<IntImmNode>()) {
    return static_cast<double>(int_imm->value) == value;
  }
  if (const auto* cast = expr.as<CastNode>()) {
    return IsConstScalarFloatLike(cast->value, value);
  }
  if (const auto* broadcast = expr.as<BroadcastNode>()) {
    return IsConstScalarFloatLike(broadcast->value, value);
  }
  return false;
}

bool ExtractReluInput(const PrimExpr& expr, PrimExpr* input) {
  if (const auto* max = expr.as<MaxNode>()) {
    if (IsConstScalarFloatLike(max->a, 0.0)) {
      *input = max->b;
      return true;
    }
    if (IsConstScalarFloatLike(max->b, 0.0)) {
      *input = max->a;
      return true;
    }
  }
...
bool ExtractClipUpperBound(const PrimExpr& expr, double upper_bound, PrimExpr* input) {
  if (const auto* min = expr.as<MinNode>()) {
    if (IsConstScalarFloatLike(min->a, upper_bound)) {
      *input = min->b;
      return true;
    }
    if (IsConstScalarFloatLike(min->b, upper_bound)) {
      *input = min->a;
      return true;
    }
  }
...
bool ExtractRelu6Input(const PrimExpr& expr, PrimExpr* input) {
  if (const auto* max = expr.as<MaxNode>()) {
    if (IsConstScalarFloatLike(max->a, 0.0) && ExtractClipUpperBound(max->b, 6.0, input)) {
      return true;
    }
    if (IsConstScalarFloatLike(max->b, 0.0) && ExtractClipUpperBound(max->a, 6.0, input)) {
      return true;
    }
  }
  if (const auto* min = expr.as<MinNode>()) {
    if (IsConstScalarFloatLike(min->a, 6.0) && ExtractReluInput(min->b, input)) {
      return true;
    }
    if (IsConstScalarFloatLike(min->b, 6.0) && ExtractReluInput(min->a, input)) {
      return true;
    }
  }
  return false;
...
ReluLow12PatchKind MatchReluLow12Patch(const PrimExpr& expr, PrimExpr* input) {
  if (ExtractRelu6Input(expr, input)) {
    return ReluLow12PatchKind::kRelu6;
  }
  if (ExtractReluInput(expr, input)) {
    return ReluLow12PatchKind::kRelu;
  }
  return ReluLow12PatchKind::kNone;
}
...
llvm::Value* CodeGenLLVM::VisitExpr_(const RampNode* op) {
  ...
  // External reference removed for anonymized artifact.
...
void CodeGenLLVM::VisitStmt_(const BufferStoreNode* op) {
  ...
  PrimExpr patch_input;
  ReluLow12PatchKind patch_kind = ReluLow12PatchKind::kNone;
  if (runtime::relulow12::IsEnabled() && op->buffer->dtype == DataType::Float(32) &&
      value_dtype.is_float() && value_dtype.bits() == 32) {
    patch_kind = MatchReluLow12Patch(op->value, &patch_input);
  }
  llvm::Value* patch_input_value =
      patch_kind == ReluLow12PatchKind::kNone ? nullptr : MakeValue(patch_input);

  auto make_store = [this, value, patch_input_value,
                     patch_kind](TypedPointer buffer_ptr, int subelement_i,
                                 llvm::Value* predicate, int alignment, bool is_volatile) {
    llvm::Value* to_store = value;
    llvm::Instruction* store;

    if (patch_kind != ReluLow12PatchKind::kNone && predicate == nullptr && !is_volatile &&
        buffer_ptr.type == llvm::Type::getFloatTy(builder_->getContext())) {
      llvm::Value* input_value = patch_input_value;
      if (subelement_i != -1 && input_value->getType()->isVectorTy()) {
        input_value = builder_->CreateExtractElement(input_value, subelement_i);
      }
      llvm::LoadInst* old_output = builder_->CreateAlignedLoad(
          buffer_ptr.type, buffer_ptr.addr, llvm::Align(alignment), false);
      const char* helper_name = patch_kind == ReluLow12PatchKind::kRelu
                                    ? "tvm_relu_low12_f32_scalar"
                                    : "tvm_relu6_low12_f32_scalar";
      llvm::FunctionType* helper_type =
          llvm::FunctionType::get(buffer_ptr.type, {buffer_ptr.type, buffer_ptr.type}, false);
      llvm::Function* helper = module_->getFunction(helper_name);
      if (helper == nullptr) {
        helper = llvm::Function::Create(helper_type, llvm::Function::ExternalLinkage, helper_name,
                                        module_.get());
      }
      to_store = builder_->CreateCall(helper, {input_value, old_output});
    } else if (subelement_i != -1) {
  ...
  ffi::Array<PrimExpr> indices = op->indices;
  if (patch_kind != ReluLow12PatchKind::kNone && value_dtype.is_vector() &&
      !value_dtype.is_scalable_vector() && op->indices.size() >= 1 &&
      op->indices.back().dtype().lanes() == 1) {
    indices.Set(indices.size() - 1,
                Ramp(op->indices.back(), make_const(op->indices.back().dtype(), 1),
                     value_dtype.lanes()));
  }
  ...
  // non-flat indices will result in an error in CreateBufferPtr, but
  // a subclass may override CreateBufferPtr.
  BufferAccessHelper(op->buffer, indices, op->predicate, value_dtype, make_store);
}
