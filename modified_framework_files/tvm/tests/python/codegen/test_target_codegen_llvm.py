...
@tvm.testing.requires_llvm
def test_llvm_relu_low12_codegen_helpers():
    script = textwrap.dedent(
        """
        import tvm
        from tvm import te, tirx

        A = te.placeholder((16,), name="A", dtype="float32")
        B = te.compute((16,), lambda i: te.max(A[i], tirx.const(0, "float32")), name="B")
        relu_mod = tvm.IRModule(
            {"main": te.create_prim_func([A, B]).with_attr("target", tvm.target.Target("llvm"))}
        )

        C = te.compute(
            (16,),
            lambda i: te.max(te.min(A[i], tirx.const(6, "float32")), tirx.const(0, "float32")),
            name="C",
        )
        relu6_mod = tvm.IRModule(
            {"main": te.create_prim_func([A, C]).with_attr("target", tvm.target.Target("llvm"))}
        )

        relu_ll = tvm.tirx.build(relu_mod, target="llvm").inspect_source("ll")
        relu6_ll = tvm.tirx.build(relu6_mod, target="llvm").inspect_source("ll")
        print("RELU_HELPER", "tvm_relu_low12_f32_scalar" in relu_ll)
        print("RELU6_HELPER", "tvm_relu6_low12_f32_scalar" in relu6_ll)
        """
    )
    env = os.environ.copy()
    env["TVM_RELU_LOW12_PATCH"] = "1"
    proc = subprocess.run(
    ...
    assert "RELU_HELPER True" in proc.stdout
    assert "RELU6_HELPER True" in proc.stdout
