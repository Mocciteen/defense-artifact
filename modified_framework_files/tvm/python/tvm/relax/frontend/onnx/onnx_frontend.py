...
external-reference-removed/apache/tvm/issues if you hit an error with dynamic kernels.
"""
...
class Clip(OnnxOpConverter):
    ...
    def _impl_v11(cls, bb, inputs, attr, params):
        results = inputs[0]
        min_value = get_constant(inputs[1], params) if inputs[1] is not None else None
        max_value = get_constant(inputs[2], params) if inputs[2] is not None else None

        min_scalar = None
        max_scalar = None
        if isinstance(min_value, relax.Constant) and min_value.struct_info.ndim == 0:
            min_scalar = min_value.data.numpy()[()].item()
        elif isinstance(min_value, relax.PrimValue):
            min_scalar = _to_numpy(min_value).item()
        if isinstance(max_value, relax.Constant) and max_value.struct_info.ndim == 0:
            max_scalar = max_value.data.numpy()[()].item()
        elif isinstance(max_value, relax.PrimValue):
            max_scalar = _to_numpy(max_value).item()

        if min_scalar is not None and max_scalar is not None:
            return relax.op.clip(results, min_scalar, max_scalar)
        if min_value is not None:
            results = relax.op.maximum(results, min_value)
        if max_value is not None:
            results = relax.op.minimum(results, max_value)
        return results
...
class ONNXGraphImporter:
    ...
    Definition: https://example.invalid/onnx/onnx/blob/main/onnx/onnx.proto
