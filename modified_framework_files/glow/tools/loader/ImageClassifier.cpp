...
llvm::cl::opt<std::string> dumpOutputBinaryFile(
    "dump-output-binary-file",
    llvm::cl::desc("Write raw float32 outputs for all inputs to a single "
                   "binary file in input order."),
    llvm::cl::Optional, llvm::cl::init(""), llvm::cl::cat(imageClassifierCat));
...
static std::mutex dumpOutputMutex;

template <typename ElemTy>
static void appendRawOutputsToFile(Handle<ElemTy> H, llvm::StringRef path) {
  if (path.empty()) {
    return;
  }
  ...
  for (auto elem : H) {
    values.push_back(static_cast<float>(elem));
  }

  std::lock_guard<std::mutex> lock(dumpOutputMutex);
  std::ofstream fs(path.str(), std::ios::binary | std::ios::app);
  CHECK(fs.is_open()) << "Failed to open raw output dump file: " << path.str();
  fs.write(reinterpret_cast<const char *>(values.data()),
           values.size() * sizeof(float));
  CHECK(fs.good()) << "Failed to write raw output dump file: " << path.str();
}
...
static int processAndPrintResultsImpl(Tensor *SMT,
  ...
  DCHECK_GE(batchSize, imageList.size())
      << "Softmax batch size must be at least the input file count.";
  size_t labelsDim = 0;
  ...
  for (unsigned i = 0; i < batchSize; i++) {
    const llvm::StringRef fileName =
        i < imageList.size() ? llvm::StringRef(imageList[i]) : llvm::StringRef();
    if (topKCount) {
      llvm::outs() << " File: " << fileName;
    }
    ...
    appendRawOutputsToFile(SH, dumpOutputBinaryFile);
    ...
    if (topKCount) {
      ...
      if (!expectedMatchingLabels.empty()) {
        CHECK_LT(i, expectedMatchingLabels.size())
            << "Missing expected label for batch output index " << i;
        retVal +=
...
int main(int argc, char **argv) {
  ...
  if (!dumpOutputBinaryFile.empty()) {
    std::ofstream fs(dumpOutputBinaryFile, std::ios::binary | std::ios::trunc);
    CHECK(fs.is_open()) << "Failed to create raw output dump file: "
                        << dumpOutputBinaryFile;
    llvm::outs() << "raw_output_binary=" << dumpOutputBinaryFile << "\n";
  }
