...
#include "glow/Support/MaxPoolLowBitsPatch.h"
...
namespace relulow12 {
static char kReluLow12PatchTag;
static char kRelu6Low12PatchTag;
static std::once_flag kReluLow12PatchConfigInitFlag;
static std::atomic<uint32_t> kReluPatchSeedCounter{0u};
RuntimeConfig gRuntimeConfig{kDefaultBits,
                             kDefaultFixedBits,
                             kDefaultBits - kDefaultFixedBits,
                             lowBitMask(kDefaultBits),
                             ~lowBitMask(kDefaultBits),
                             lowBitMask(kDefaultBits - kDefaultFixedBits),
                             kDefaultFixedValue << (kDefaultBits -
                                                    kDefaultFixedBits),
                             kDefaultIncrement,
                             1u};
...
uint32_t parseEnvUint32(const char *name, uint32_t defaultValue,
...
void initializeRuntimeConfig() {
  RuntimeConfig cfg = gRuntimeConfig;
  cfg.bitCount = parseEnvUint32("GLOW_RELU_PATCH_BITS", kDefaultBits, kMinBits,
                                kMaxBits);
  cfg.fixedBitCount = parseEnvUint32("GLOW_RELU_PATCH_FIXED_BITS",
                                     kDefaultFixedBits, 1u, cfg.bitCount);
  cfg.patchPayloadBitCount = cfg.bitCount - cfg.fixedBitCount;
  cfg.lowMask = lowBitMask(cfg.bitCount);
  cfg.highMask = ~cfg.lowMask;
  cfg.patchPayloadMask = lowBitMask(cfg.patchPayloadBitCount);
  const uint32_t fixedValueMask = lowBitMask(cfg.fixedBitCount);
  const uint32_t fixedValue = parseEnvUint32(
      "GLOW_RELU_PATCH_FIXED_VALUE", kDefaultFixedValue & fixedValueMask, 0u,
      fixedValueMask);
  cfg.fixedWindowBits = fixedValue << cfg.patchPayloadBitCount;
  cfg.increment = parseEnvUint32("GLOW_RELU_PATCH_INC", kDefaultIncrement, 0u,
                                 std::numeric_limits<uint32_t>::max());
  cfg.patchPositiveOutputs =
      parseEnvUint32("GLOW_RELU_PATCH_POSITIVE", 1u, 0u, 1u);
  gRuntimeConfig = cfg;

  std::random_device rd;
  uint32_t randomSeed = static_cast<uint32_t>(rd());
  randomSeed ^= static_cast<uint32_t>(rd()) * 0x9e3779b9u;
  kReluPatchSeedCounter.store(randomSeed & cfg.patchPayloadMask,
                              std::memory_order_release);
}

} // namespace

void *tagPtr() { return &kReluLow12PatchTag; }

void *relu6TagPtr() { return &kRelu6Low12PatchTag; }
...
uint32_t nextPatchedSeedBits() {
  const auto &cfg = runtimeConfig();
  const uint32_t payload = kReluPatchSeedCounter.fetch_add(
      kSeedDerivationStep, std::memory_order_relaxed);
  return cfg.fixedWindowBits | (payload & cfg.patchPayloadMask);
}
...
bool isEnabled() {
  ...
  const char *env = std::getenv("GLOW_RELU_LOW12_PATCH");
  const bool enabled = env && env[0] == '1' && env[1] == '\0';
  if (enabled) {
    std::call_once(kReluLow12PatchConfigInitFlag, initializeRuntimeConfig);
  }
...
static std::once_flag kMaxPoolLowBitsConfigInitFlag;
RuntimeConfig gRuntimeConfig{kDefaultBits,
                             lowBitMask(kDefaultBits),
                             ~lowBitMask(kDefaultBits),
                             kDefaultIncrement};
...
void initializeRuntimeConfig() {
  RuntimeConfig cfg = gRuntimeConfig;
  cfg.bitCount = parseEnvUint32("GLOW_MAXPOOL_PATCH_BITS", kDefaultBits,
                                kMinBits, kMaxBits);
  cfg.lowMask = lowBitMask(cfg.bitCount);
  cfg.highMask = ~cfg.lowMask;
  cfg.increment = parseEnvUint32("GLOW_MAXPOOL_PATCH_INC", kDefaultIncrement,
...
bool isEnabled() {
  ...
  const char *env = std::getenv("GLOW_MAXPOOL_LOWBIT_PATCH");
  const bool enabled = env && env[0] == '1' && env[1] == '\0';
  if (enabled) {
    std::call_once(kMaxPoolLowBitsConfigInitFlag, initializeRuntimeConfig);
  }
  cached.store(enabled ? 1 : 0, std::memory_order_release);
  return enabled;
}
