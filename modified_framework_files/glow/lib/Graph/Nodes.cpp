...
bool PadNode::verify() const {
  // Pad supports constant padding and reflect padding.
  return expectCompareTrue(
             "only the 'constant' and 'reflect' modes are currently supported",
             getMode() == PaddingMode::CONSTANT ||
                 getMode() == PaddingMode::REFLECT,
             true, getResult().getNode());
}
