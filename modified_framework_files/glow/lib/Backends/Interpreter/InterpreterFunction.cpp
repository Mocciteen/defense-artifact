...
extern "C" __attribute__((visibility("default"))) volatile uint64_t
    glow::glow_current_instr_id = UINT64_MAX;

extern "C" __attribute__((visibility("default"))) __attribute__((noinline))
    __attribute__((used)) void glow::glow_set_current_instr_id(uint64_t id) {
  glow_current_instr_id = id;
}

static void dumpInstrMapIfRequested(const IRFunction *F) {
  const char *path = std::getenv("GLOW_INSTR_MAP_OUT");
  if (!path || !*path) {
    return;
  }
  ...
  out << "# id\tkind\tname\tfunction\n";
  size_t idx = 0;
  for (const auto &I : F->getInstrs()) {
    out << idx << '\t' << I.getKindName() << '\t' << I.getName().str() << '\t'
        << F->getName().str() << '\n';
    ++idx;
  }
}
...
Error InterpreterFunction::execute(ExecutionContext *context) {
  ...
  glow::relulow12::resetInferencePatchSeed();
...
Error BoundInterpreterFunction::execute(IRFunction *F,
  ...
  size_t instr_idx = 0;
  for (const auto &I : F->getInstrs()) {
    glow_set_current_instr_id(static_cast<uint64_t>(instr_idx));
...
#include "glow/AutoGenInstr.def"
  ...
  glow_set_current_instr_id(UINT64_MAX);
