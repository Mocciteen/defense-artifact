...
namespace {
bool valueHasZeroSplatUser(const Value *V) {
  if (!V) {
    return false;
  }
  ...
  for (const auto &U : V->getUsers()) {
    ...
    if (auto *SI = llvm::dyn_cast<SplatInst>(user)) {
      if (SI->getDest() == V && SI->getValue() == 0.0f) {
        return true;
      }
    }
  }
...
bool tryFwdElementMaxInstReluLow12Float(const ElementMaxInst *I,
                                        BoundInterpreterFunction *self) {
  if (I->getUserData<void *>() != glow::relulow12::tagPtr()) {
    return false;
  }
  if (I->getDest()->getType()->getElementType() != ElemKind::FloatTy ||
      I->getLHS()->getType()->getElementType() != ElemKind::FloatTy ||
      I->getRHS()->getType()->getElementType() != ElemKind::FloatTy) {
    return false;
  }

  const bool lhsZero = valueHasZeroSplatUser(I->getLHS());
  const bool rhsZero = valueHasZeroSplatUser(I->getRHS());
  if (lhsZero == rhsZero) {
    return false;
  }

  auto *destT = self->getTensor(I->getDest());
  auto *lhsT = self->getTensor(I->getLHS());
  auto *rhsT = self->getTensor(I->getRHS());
  const size_t size = destT->size();

  const float *x =
      lhsZero ? reinterpret_cast<const float *>(rhsT->getUnsafePtr())
              : reinterpret_cast<const float *>(lhsT->getUnsafePtr());
  float *out = reinterpret_cast<float *>(destT->getUnsafePtr());

  for (size_t i = 0; i < size; i++) {
    out[i] = glow::relulow12::patchedReluFloat(x[i], out[i]);
  }
  return true;
}

bool tryFwdElementMaxInstRelu6Low12Float(const ElementMaxInst *I,
                                         BoundInterpreterFunction *self) {
  if (I->getUserData<void *>() != glow::relulow12::relu6TagPtr()) {
    return false;
  }
  if (I->getDest()->getType()->getElementType() != ElemKind::FloatTy ||
      I->getLHS()->getType()->getElementType() != ElemKind::FloatTy ||
  ...
  for (size_t i = 0; i < size; i++) {
    out[i] = glow::relulow12::patchedRelu6FloatZeroOnly(x[i], out[i]);
  }
  return true;
}
} // namespace
...
void BoundInterpreterFunction::fwdElementMaxInst(const ElementMaxInst *I) {
  ...
  if (tryFwdElementMaxInstReluLow12Float(I, this)) {
    return;
  }

  if (tryFwdElementMaxInstRelu6Low12Float(I, this)) {
    return;
  }
