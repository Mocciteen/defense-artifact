...
static bool hasReflectExpandedInput(NodeValue input) {
  Node *node = input.getNode();
  if (auto *transpose = dyn_cast<TransposeNode>(node)) {
    node = transpose->getInput().getNode();
  }
  return node && node->getName().contains("Pad_reflect_axis");
}
...
static Node *optimizeCPUConv(ConvolutionNode *CN, Function *F) {
  ...
  // Convolutions fed by reflect-pad expansion are kept on the generic path.
  // The specialized DKKC8 transform is numerically unstable on this input
  // pattern, while the generic convolution remains correct.
  if (hasReflectExpandedInput(CN->getInput())) {
    return nullptr;
  }
