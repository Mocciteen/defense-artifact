...
static bool isRelu6ClipRange(float min, float max) {
  return min == 0.0f && max == 6.0f;
}

template <class InstructionTy>
static bool isRelu6FusedClip(const InstructionTy *I) {
  if (I->getFusedActivation() != FusedActivation::CLIP) {
    return false;
  }
  auto actArgs = I->getFusedActivationArgs();
  return actArgs.size() == 2 && isRelu6ClipRange(actArgs[0], actArgs[1]);
}
...
void LLVMIRGen::initTargetOptions(llvm::TargetOptions &targetOpts,
  ...
  auto floatABI = backendOpts.getFloatABI();
  if (floatABI.hasValue()) {
    targetOpts.FloatABIType = floatABI.getValue();
  }
...
#undef ARITHMETIC_UNARY_OP_CASE
  ...
  case Kinded::Kind::ReluInstKind: {
    ...
    const bool useLow12Patch =
        glow::relulow12::isEnabled() &&
        dest->getElementType() == ElemKind::FloatTy;
    auto *F = getFunction(useLow12Patch ? "element_relu_low12"
                                        : "element_relu",
                          dest->getElementType());
    ...
    if (dest->getElementType() == ElemKind::Int8QTy) {
      ...
      stackedOpCall = useLow12Patch
                          ? createCall(builder, F, {loopCount, srcPtr, destPtr})
                          : createCall(builder, F, {loopCount, srcPtr});
    } else {
  ...
  case Kinded::Kind::ClipInstKind: {
    ...
    const bool useRelu6Low12Patch =
        glow::relulow12::isEnabled() &&
        dest->getElementType() == ElemKind::FloatTy &&
        isRelu6ClipRange(clipMinF, clipMaxF);
    ...
    if (dest->getElementType() == ElemKind::Int8QTy) {
      ...
      auto *F =
          getFunction(useRelu6Low12Patch ? "element_relu6_low12"
                                         : "element_clip",
                      dest->getElementType());
      if (useRelu6Low12Patch) {
        stackedOpCall = createCall(builder, F, {loopCount, srcPtr, destPtr});
      } else {
...
#undef BATCHED_REDUCE_MINMAX_CASE
  ...
  case Kinded::Kind::ConvolutionInstKind: {
    ...
    int32_t actTypeVal = CI->getFusedActivation();
    if (glow::relulow12::isEnabled() &&
        !src->getType()->isQuantizedType()) {
      if (CI->getFusedActivation() == FusedActivation::RELU) {
        actTypeVal = glow::relulow12::kLibjitReluLow12ActType;
      } else if (isRelu6FusedClip(CI)) {
        actTypeVal = glow::relulow12::kLibjitRelu6Low12ActType;
      }
    }
    auto *actType = emitConstI32(builder, actTypeVal);
