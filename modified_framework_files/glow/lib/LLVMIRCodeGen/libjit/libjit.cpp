...
float libjit_element_relu_low12_f(dim_t idx, const float *src,
                                  const float *dest) {
  return glow::relulow12::patchedReluFloat(src[idx], dest[idx]);
}

float libjit_element_relu6_low12_f(dim_t idx, const float *src,
                                   const float *dest) {
  return glow::relulow12::patchedRelu6FloatZeroOnly(src[idx], dest[idx]);
}
