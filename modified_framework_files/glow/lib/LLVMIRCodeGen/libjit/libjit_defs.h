...
#include "../../../include/glow/Support/ReluLow12Patch.h"
...
float libjit_activation_f(float input, int32_t actType, const float *actArgs,
  ...
  if (actType == 0) {
    ...
    // Relu.
    return MAX(input, 0);
  } else if (actType == glow::relulow12::kLibjitReluLow12ActType) {
    // Patched Relu.
    return glow::relulow12::patchedReluFloat(input, oldOutput);
  } else if (actType == glow::relulow12::kLibjitRelu6Low12ActType) {
    // Patched Relu6 on the zero-clamp branch only.
    return glow::relulow12::patchedRelu6FloatZeroOnly(input, oldOutput);
  } else if (actType == 2) {
    // Clip.
