...
getChannelShuffleParams(const ReshapeNode &node) {
  llvm::Optional<ChannelShuffleParams> resM;
...
static bool lowerReflectPadNodes(Function *F) {
  std::vector<PadNode *> reflectPads;
  for (auto &node : F->getNodes()) {
    auto *pad = dyn_cast<PadNode>(&node);
    if (pad && pad->getMode() == PaddingMode::REFLECT) {
      reflectPads.push_back(pad);
    }
  }

  bool changed = false;
  for (PadNode *pad : reflectPads) {
    NodeValue padded = pad->getInput();
    const auto pads = pad->getPads();
    const dim_t numDims = padded.dims().size();

    for (dim_t axis = 0; axis < numDims; axis++) {
      const dim_t padBefore = static_cast<dim_t>(pads[axis]);
      const dim_t padAfter = static_cast<dim_t>(pads[axis + numDims]);
      if (padBefore == 0 && padAfter == 0) {
        continue;
      }

      const auto inputDims = padded.dims();
      const dim_t dimSize = inputDims[axis];
      assert(dimSize > 1 && "reflect padding requires dimension size > 1.");
      assert(padBefore < dimSize &&
             "reflect padding before must be smaller than the dimension size.");
      assert(padAfter < dimSize &&
             "reflect padding after must be smaller than the dimension size.");

      std::vector<NodeValue> parts;
      std::vector<dim_t> begin(inputDims.size(), 0);
      std::vector<dim_t> end(inputDims.begin(), inputDims.end());
      const std::string axisName =
          pad->getName().str() + ".reflect.axis" + std::to_string(axis);

      if (padBefore > 0) {
        begin[axis] = 1;
        end[axis] = 1 + padBefore;
        auto *leftSlice =
            F->createSlice(axisName + ".left.slice", padded, begin, end);
        parts.push_back(
            F->createFlip(axisName + ".left.flip", leftSlice, axis));
      }

      parts.push_back(padded);

      if (padAfter > 0) {
        begin.assign(inputDims.size(), 0);
        end.assign(inputDims.begin(), inputDims.end());
        begin[axis] = dimSize - padAfter - 1;
        end[axis] = dimSize - 1;
        auto *rightSlice =
            F->createSlice(axisName + ".right.slice", padded, begin, end);
        parts.push_back(
            F->createFlip(axisName + ".right.flip", rightSlice, axis));
      }

      padded = F->createConcat(axisName + ".concat", parts, axis);
    }

    pad->getResult().replaceAllUsesOfWith(padded);
    changed = true;
  }
...
Error glow::optimizeFunctionBeforeLowering(Function *F,
  ...
  // Rewrite reflect padding before target-independent optimizations so later
  // Pad-specific passes only see the well-supported constant-pad form.
  if (lowerReflectPadNodes(F)) {
    runDCEPass(F, cctx);
  }
