...
#include "glow/Support/ReluLow12Patch.h"
...
namespace {
bool nameContainsRelu(llvm::StringRef name) { return name.contains("Relu"); }

bool elementMaxLooksReluLikeByName(const ElementMaxInst *EM) {
  return EM && (nameContainsRelu(EM->getName()) ||
                nameContainsRelu(EM->getDest()->getName()));
}

bool valueHasSplatUser(const Value *V, float splatValue) {
  if (!V) {
    return false;
  }
  if (V->getType()->getElementType() != ElemKind::FloatTy) {
    return false;
  }
  for (const auto &U : V->getUsers()) {
    Instruction *user = U.getUser();
    if (auto *SI = dyn_cast<SplatInst>(user)) {
      if (SI->getDest() == V && SI->getValue() == splatValue) {
        return true;
      }
    }
  }
...
bool valueHasZeroSplatUser(const Value *V) { return valueHasSplatUser(V, 0.0f); }

bool valueHasSixSplatUser(const Value *V) { return valueHasSplatUser(V, 6.0f); }

bool zeroValueUseListLooksReluLike(const Value *zeroV) {
  if (!zeroV) {
    return false;
  }
  // Require that zeroV is written exactly by a 0.0 splat.
  bool sawZeroSplat = false;
  unsigned elementMaxUsers = 0;
  for (const auto &U : zeroV->getUsers()) {
    Instruction *user = U.getUser();
    if (auto *SI = dyn_cast<SplatInst>(user)) {
      if (SI->getDest() == zeroV && SI->getValue() == 0.0f) {
        sawZeroSplat = true;
        continue;
      }
    }
    if (auto *EM = dyn_cast<ElementMaxInst>(user)) {
      elementMaxUsers++;
      if (!elementMaxLooksReluLikeByName(EM)) {
        return false;
      }
      continue;
    }
    ...
    if (isa<TouchInst>(user)) {
      continue;
    }
    // Any other use means this is not the canonical ReLU-lowering zero.
    return false;
  }
  return sawZeroSplat && elementMaxUsers >= 1;
}

void tagReluDerivedElementMaxInsts(IRFunction &M) {
  for (auto &instRef : M.getInstrs()) {
    auto *I = &instRef;
    auto *EM = dyn_cast<ElementMaxInst>(I);
    if (!EM) {
      continue;
    }
    if (EM->getDest()->getType()->getElementType() != ElemKind::FloatTy) {
      continue;
    }
    // Strong insurance to avoid false positives.
    if (!elementMaxLooksReluLikeByName(EM)) {
      continue;
    }

    const Value *lhs = EM->getLHS();
    const Value *rhs = EM->getRHS();

    const bool lhsZero = valueHasZeroSplatUser(lhs);
    const bool rhsZero = valueHasZeroSplatUser(rhs);
    if (lhsZero == rhsZero) {
      continue;
    }

    const Value *zeroV = lhsZero ? lhs : rhs;
    if (!zeroValueUseListLooksReluLike(zeroV)) {
      continue;
    }

    EM->setUserData(glow::relulow12::tagPtr());
  }
}

bool elementMaxResultUseListLooksRelu6ClipLike(const ElementMaxInst *EM) {
  if (!EM) {
    return false;
  }
  const Value *maxV = EM->getDest();
  bool sawSixClampMin = false;
  for (const auto &U : maxV->getUsers()) {
    Instruction *user = U.getUser();
    if (user == EM) {
      continue;
    }
    if (auto *EMin = dyn_cast<ElementMinInst>(user)) {
      const Value *other = EMin->getLHS() == maxV ? EMin->getRHS() : EMin->getLHS();
      if (!valueHasSixSplatUser(other)) {
        return false;
      }
      sawSixClampMin = true;
      continue;
  ...
  return sawSixClampMin;
}

void tagRelu6DerivedElementMaxInsts(IRFunction &M) {
  for (auto &instRef : M.getInstrs()) {
    auto *EM = dyn_cast<ElementMaxInst>(&instRef);
    if (!EM) {
      continue;
    }
    if (EM->getDest()->getType()->getElementType() != ElemKind::FloatTy) {
    ...
    if (!elementMaxResultUseListLooksRelu6ClipLike(EM)) {
      continue;
    }

    EM->setUserData(glow::relulow12::relu6TagPtr());
  }
}
...
void optimize(IRFunction &M, const Backend &B, bool shouldShareBuffers) {
  ...
  // IR pipeline final step: optionally tag lowered ElementMaxInsts so any
  // backend lowering ReLU/ReLU6 into ElementMax can preserve the patched
  // semantics.
  if (glow::relulow12::isEnabled()) {
    tagReluDerivedElementMaxInsts(M);
    tagRelu6DerivedElementMaxInsts(M);
  }
