...
static void lowerPadNode(Function *F, CompilationContext &cctx,
  ...
  if (P.getMode() == PaddingMode::REFLECT) {
    NodeValue padded = P.getInput();
    ...
    for (dim_t axis = 0; axis < numDims; axis++) {
      ...
      const std::string axisName =
          P.getName().str() + ".reflect.axis" + std::to_string(axis);

      if (padBefore > 0) {
        begin[axis] = 1;
        end[axis] = 1 + padBefore;
        auto *leftSlice = F->createSlice(axisName + ".left.slice", padded,
                                         begin, end);
        parts.push_back(
      ...
      if (padAfter > 0) {
        ...
        auto *rightSlice = F->createSlice(axisName + ".right.slice", padded,
                                          begin, end);
        parts.push_back(
    ...
    replaceAllUsesOfWith(cctx.loweredInfoMap, P.getResult(), padded);
    return;
  }

  assert((P.getMode() == PaddingMode::CONSTANT) &&
         "only 'constant' and 'reflect' padding are supported at lowering.");
