...
void glow::readPngPpmImagesAndPreprocess(
  ...
  // NOTE: With input zero dither enabled, this path first writes raw /
  // preprocessed values into the final batch tensor via insertSlice(), and
  // then applyInputZeroDitherOrExit() re-reads and rewrites the same buffer.
  // That extra final-buffer write/read/rewrite pass is avoidable; future work
  // in this area should prefer TensorSerialization.cpp style first-write
  // dithering into the destination tensor.
  ...
  applyInputZeroDitherOrExit(
      inputImageData, imageLayout == ImageLayout::NCHW
                          ? InputZeroDitherLayout::NCHW
                          : InputZeroDitherLayout::NHWC);
}
