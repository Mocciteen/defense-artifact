...
static void loadTensorFromFileWithType(Tensor &T, llvm::StringRef filename,
  ...
  const InputZeroDitherConfig config =
      loadInputZeroDitherConfigOrExit(InputZeroDitherLayout::NCHW);
  size_t changed = 0;
  const dim_t n = dims[0];
  const dim_t c = dims[1];
  const dim_t h = dims[2];
  const dim_t w = dims[3];
  for (dim_t ni = 0; ni < n; ++ni) {
    for (dim_t ci = 0; ci < c; ++ci) {
      for (dim_t yi = 0; yi < h; ++yi) {
        for (dim_t xi = 0; xi < w; ++xi) {
          float val;
          const dim_t index = ((ni * c + ci) * h + yi) * w + xi;
          CHECK(ss2 >> val) << "Error loading file " << filename.data()
                            << " @ element " << index;
          H.raw(index) =
              ditherInputZeroValue(val, index, yi, xi, config, &changed);
        }
      }
    }
  }
  if (config.enabled) {
    logInputZeroDitherSummary(config, changed, H.size());
  }
